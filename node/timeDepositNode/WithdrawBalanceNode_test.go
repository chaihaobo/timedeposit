 ...
 * @Last Modified time: 2022-05-23 09:46:16
 */
...
func TestWithdrawBalanceNode_RunProcess(t *testing.T) {
	type fields struct {
		Node node.Node
	}
	type args struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("WithdrawBalanceNode.RunProcess() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("WithdrawBalanceNode.RunProcess() = %v, want %v", got, tt.want)
			}
		})
	}
}
